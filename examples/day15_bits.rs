...
impl<'a> Parser<'a> {
    ...
    fn next_n_bits_be(&mut self, n: u32) -> u32 {
        let mut v = 0;
        for _ in 0..n {
            v <<= 1;
            v |= self.next_bit() as u32;
        }
        v
    }

    fn next_version(&mut self) -> u32 {
        self.next_n_bits_be(3)
    }
    ...
    fn next_len(&mut self) -> Len {
        match self.next_len_type_id() {
            LenTypeId::Bits => Len::Bits(self.next_n_bits_be(15)),
            LenTypeId::Packets => Len::Packets(self.next_n_bits_be(11)),
        }
    }
    ...
    fn next_packet_content(&mut self, type_id: u32) -> PacketContent {
        if type_id == 4 {
